...
- 扫描与诊断默认只读；可逆的停用、恢复和配置行操作必须经过明确确认、完整性校验和独立恢复路径。
...
- [VERIFY-001] 修改后运行仓库声明的统一验证入口；涉及页面流程时优先执行仓库既有、通过 mise 等标准入口声明的 E2E 或烟测。
- [TEST-001] 测试按风险添加，优先覆盖非平凡逻辑、稳定契约、安全边界和缺陷回归；静态配置与简单透传通常使用语法检查、构建或验收验证，不因每次改动机械新增测试。
- [TEST-002] 不为 UI 展现层编写单元测试或快照测试；界面外观与页面交互优先通过仓库既有标准烟测覆盖，可脱离 UI 的业务逻辑仍按风险编写单元测试。
- [TEST-003] 可以执行仓库既有、通过 mise 等标准入口声明的 E2E 或烟测；除非用户当次明确要求，不得临时编写、拼接或手动执行自定义 E2E，包括 AppleScript / 辅助功能 UI 自动化、自行打开应用截图比较和临时截图脚本，也不得新增或扩写 E2E 测试与脚本。
